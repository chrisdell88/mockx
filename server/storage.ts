...
export class DatabaseStorage implements IStorage {
  ...
  async getAccuracyLeaderboard(_minSiteYears = 3): Promise<AccuracyRow[]> {
    ...
    // Anyone not meeting these has x_score = NULL, so `a.x_score IS NOT NULL`
    // below is sufficient. The minSiteYears arg is accepted for backward
    // compatibility with callers but is no longer used for filtering — the DB
    // column does it.
